...
"""
Implementation of Kosaraju's algorithm for computing the strongly connected components of a directed graph.
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compute strongly connected components of a directed graph.")
    parser.add_argument('filename', type=str, help="file containing graph")
    parser.add_argument('k', type=int, help="number of SCCs' sizes to show, from largest to smallest")

    args = parser.parse_args()

    edge_list = parse_file(args.filename)
    g = Graph()
    g.parse_edge_list(edge_list)
    k = Kosaraju(g)
    k.run()
    k.show_sccs(args.k)